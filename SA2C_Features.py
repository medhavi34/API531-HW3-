...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, item_features, mixing_parameter, pretrain, name='DQNetwork'):
        ...
        with tf.variable_scope(self.name):
            ...
            self.output2= tf.layers.dense(inputs=self.states_hidden, units=self.item_num,
                               activation=None, name="ce-logits")   
            ...
            ) * self.output2 + self.mixing_parameter * self.phi_hk_fj  ####  all ce logits
            ...
            ce_loss_pre = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.actions, logits=self.score)
...
if __name__ == '__main__':
    ...
    item_features = pd.read_csv(
        os.path.join("item_features.csv")
    ...
    QN_1 = QNetwork(name='QN_1', hidden_size=args.hidden_factor, learning_rate=args.lr, item_num=item_num,
                    state_size=state_size, item_features=item_features, mixing_parameter=mixing_parameter,
                    pretrain=False)
